...
Display *get_browser_display()
{
    return g_toplevel ? XtDisplay(g_toplevel) : NULL;
}
...
void select_tab_page(BrowserTab *tab)
{
    if (!g_tab_stack || !tab || !tab->page) return;
...
bookmark_entry_copy_icon_from_cache(BookmarkEntry *entry, const char *url)
{
    ...
    int width = 0;
    int height = 0;
    if (!TabManager::instance().getCachedFavicon(url, &raw_data, &width, &height, &png_data)) {
        return;
    }
    if (width > 0 && height > 0) {
        size_t expected_size = (size_t)width * (size_t)height * 4u;
        if (raw_data.size() >= expected_size) {
            entry->icon_raw = raw_data;
            entry->icon_width = width;
            entry->icon_height = height;
        }
    }
    if (!png_data.empty()) {
        entry->icon_png = png_data;
    }
}
...
static void surface_new_tab_handler_impl(const std::string &url, bool select)
{
    ...
    fprintf(stderr,
            ...
            url.c_str(),
            select ? 1 : 0);
    std::string label = select ? "Opening new tab" : "Opening background tab";
    set_status_label_text(label.c_str());
    TabManager::instance().updateNavigationButtons(TabManager::instance().currentTab());
    (void)select;
}
...
static void surface_tab_load_finished_handler_impl(BrowserTab *tab)
{
    ...
    std::string label = tab->current_url.empty() ? "Tab load finished" : tab->current_url;
    set_status_label_text(label.c_str());
    TabManager::instance().updateNavigationButtons(tab);
    TabManager::instance().updateReloadButton(tab);
    TabManager::instance().updateFaviconControls(tab);
